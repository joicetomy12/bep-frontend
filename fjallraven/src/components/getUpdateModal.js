...
const GetUpdateModal = ({ content }) => {
  ...
  const results = modelData.bodySelections;
  ...
  const termText = _.find(results, ['section', `${lang}`], '');
          ...
          <div className={`d-flex  ${styles.card}`}>
            ...
            >
              <div className={`py-5 ${styles.formBox}`}>
                <span className={styles.cardTitle}>{heading[lang]}</span>
                <p className={styles.cardText}>{subtitle[lang]}</p>
                ...
                <div className={styles.terms}>
                  <label className={styles.checkboxDiv}>
                    <BlockContent blocks={termText['tagline']} />
