...
const TopNavigationWelcome = ({ onClose, data }) => {
  //   const defultCountryLine = data.regionSelector.marketGroups.map(obj=>{
  //     return obj.availableMarkets[2]
  //   })
  const defaultLink =
    data.regionSelector.marketGroups[0].availableMarkets[2]
      .languageUrlDictionary;
  const arry1 = data.regionSelector.marketGroups[0].availableMarkets;
  const arry2 = data.regionSelector.marketGroups[1].availableMarkets;
  ...
  const paramArray = pageParam.length > 3 ? pageParam[3] : '';
  const paramArrayAgain = pageParam.length > 4 ? '/' + pageParam[4] : '';
  const getFirstValue = languageUrlDictionary => {
    let firstKey = Object.keys(languageUrlDictionary)[0];

    let urlValue = '';

    if (firstKey && languageUrlDictionary[firstKey] === '/eu/en-gb') {
      urlValue = '/' + paramArray + paramArrayAgain;
    } else if (firstKey && firstKey !== '/eu/en-gb/') {
      urlValue =
        languageUrlDictionary[firstKey] + '/' + paramArray + paramArrayAgain;
    } else {
      urlValue = '/';
    }
      ...
      <div className="region--container top-navigation-container-wp">
        ...
        {!showLang && (
          <div className="js-welcome-area">
            ...
            <div className="allreg">
              ...
              <p className="alltext">
                <a href={getFirstValue(defaultLink)}>All countries | €</a>{' '}
              </p>
            </div>
            <div className="divider-bar-area">
              <span className="divider--subtitle">
                <p>
                  Or <span className="semi-accent">select your country</span>
                </p>
              </span>
            </div>
          </div>
        ...
        {!showLang && (
          <div className="region--groups js-region-groups">
            <div className="region--groups-column-">
              <nav className="region--countries">
                {arry3 &&
                  //  arry3.splice(2, 1) &&
                  arry3.map((market, inIndex) => (
                    <a
                      key={inIndex}
          ...
          // group.availableMarkets.splice(2, 1) &&
